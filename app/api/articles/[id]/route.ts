...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const cookies = parseCookies({ req: { headers: { cookie: cookieHeader } } } as any)
    const token = cookies["authToken"] || request.headers.get("Authorization")?.replace("Bearer ", "")
    ...
    if (article.author !== userId) {
      return NextResponse.json({ success: false, message: "You can only edit your own articles" }, { status: 403 })
    }
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (article.author !== userId) {
      return NextResponse.json({ success: false, message: "You can only delete your own articles" }, { status: 403 })
    }
